...
get_bearer_token_from_cpd() {
  ...
  oc exec -it $SP_CONTAINER -- curl -X PATCH -L "${ZENURL}/${INSTANCE_ID}/meta" -H "$JWT_TOKEN" -k -H 'Content-Type: application/json' -d '{ "provision_status": "PROVISIONED", "service_instance_version": "24.3.0" }'
